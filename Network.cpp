...
std::vector<double> Network::gradientsCalculation(const std::vector<double>& target, const std::vector<double>& output) {
...
void Network::backpropogate(const std::vector<double>& inputData, const std::vector<double>& target) {
    ...
    std::vector<double> gradients = gradientsCalculation(target, output);
    ...
    // delta for output layer: delta = dC/dO * dA/dZ
    std::vector<double> delta_output(zValues.size());
    for (int i = 0; i < zValues.size(); i++) {

        double dAdZ = derivativeActivation_Z(zValues[i]);
        delta_output[i] = gradients[i] * dAdZ;
    }
    ...
    for (int l = layers.size() - 2; l >= 0; l--) {
        ...
        std::vector<double> zVals = currentLayer->getZValues();
        std::vector<double> dC_dZ(zVals.size(), 0.0);
        ...
        for (int i = 0; i < currentLayer->getNumNeurons(); i++) {
            ...
            double dAdZ = (zVals[i] > 0) ? 1.0 : 0.01;
            dC_dZ[i] = sum * dAdZ;
        }
